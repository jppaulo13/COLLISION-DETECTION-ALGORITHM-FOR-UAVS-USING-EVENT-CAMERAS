...
cloud,idx = read_event_file_txt(fname,0.01,True,False) 
...
#for j in range(100,150): 
    ...
    if (len(cloud_imu) == 0):
        ...
        for i in range(0,len(rect_max_x)): 
            if(rect_max_y[i]-rect_min_y[i] > 5 and rect_max_x[i]-rect_min_x[i] > 5): #Only draws a rectangle for objects with height bigger than 3 pixels
                #print ("Starts at: ", cloud[bound_1,0], "seconds and ",j,"points")           
                cont += 1
                rgb_img = cv2.rectangle(rgb_img, (int(rect_min_y[i]),int(rect_min_x[i])), (int(rect_max_y[i]),int(rect_max_x[i])), (0, 255, 0),2) 
                center_y = ((rect_max_y[i]-rect_min_y[i])//2)+(rect_min_y[i]) #Round number of the center
                center_x = ((rect_max_x[i]-rect_min_x[i])//2)+(rect_min_x[i])
                
                if (cont < 2):
                    print (len(sl),"events",int(j+1),"points")
                n = 5
                result = timeit.timeit(stmt='ego_motion(sl,height,width,cloud_imu)', globals=globals(), number=n)
                # calculate the execution time
                # get the average execution time
                if (cont < 2):
                    print(f"\tExecution time is {result / n} seconds")
                print("\tMoving Object",cont)

    
        
        #img_rect = draw_rect(cluster,rgb_img)
        #cv2.imshow('Image',img)
        #cv2.imshow('Warped Image',warp_rodrigues)
        #cv2.imshow('Image after normalizing',warp_img)
        #cv2.imshow('Clean Image',clean_img)
        #cv2.imshow('Image with Rectangle of the time window' + str(i),img_rect)
        #print ("")
